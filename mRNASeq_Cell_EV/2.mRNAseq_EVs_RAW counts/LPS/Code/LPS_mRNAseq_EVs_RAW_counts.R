...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_Cell_RNASeq/2.mRNAseq_EVs_RAW counts/LPS/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("LPS_mRNAseq_EVs_RAW counts.csv", header = TRUE)
...
#write.csv(df, file = "Unique_LPS_mRNAseq_EVs_RAW counts.csv")


df <- read.csv("Unique_LPS_mRNAseq_EVs_RAW counts.csv", header = TRUE, row.names = 1)[,-c(1)]
cleanDat <- df
dim(cleanDat)
...
numericMeta <- read.csv("LPS_mRNAseq_EVs_RAW_Traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_Evs_1", "control_Evs_2" ,"control_Evs_3", "LPS_Evs_1"    , "LPS_Evs_2"  ,   "LPS_Evs_3"    )
#########################################################CHANGE##############################################################################
