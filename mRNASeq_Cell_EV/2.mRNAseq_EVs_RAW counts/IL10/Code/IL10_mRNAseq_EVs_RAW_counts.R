...
##############CHECK##############
#Comparison:Control VS IL10 : Update your script name and comparison here + Double check the folder you are in to aviod mixing diff ex results!
rm(list = ls())
dev.off()
cat("\014")
...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/2.mRNAseq_EVs_RAW counts/IL10/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("IL10_mRNAseq_EVs_RAW_counts.csv", header = TRUE)
...
#write.csv(df, file = "Unique_IL10_mRNAseq_EVs_RAW_counts.csv")

df <- read.csv("Unique_IL10_mRNAseq_EVs_RAW_counts.csv", header = TRUE, row.names = 1)[,-c(1)]
cleanDat <- df
dim(cleanDat)
...
numericMeta <- read.csv("IL10_mRNAseq_EVs_RAW_Traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_Evs_1","control_Evs_2","control_Evs_3","IL10_Evs_1","IL10_Evs_2" , "IL10_Evs_3" )
#########################################################CHANGE##############################################################################
