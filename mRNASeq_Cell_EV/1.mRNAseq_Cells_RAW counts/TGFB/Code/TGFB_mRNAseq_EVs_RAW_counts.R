...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/1.mRNAseq_Cells_RAW counts/TGFB/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("TGFB_mRNAseq_Cells_RAW_counts.csv", header = TRUE)
...
write.csv(df, file = "Unique_TGFB_mRNAseq_Cells_RAW_counts.csv")


df <- read.csv("Unique_TGFB_mRNAseq_Cells_RAW_counts.csv", header = TRUE, row.names = 1)[,-c(1)]
cleanDat <- df
dim(cleanDat)
...
numericMeta <- read.csv("TGFB_mRNAseq_Cells_RAW_Traits.csv", header = TRUE, row.names = 1)
