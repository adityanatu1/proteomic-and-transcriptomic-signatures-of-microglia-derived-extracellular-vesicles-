...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/1.mRNAseq_Cells_RAW counts/All PCA/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("mRNAseq_Cells_RAW_counts.csv", header = TRUE)
...
#write.csv(df, file = "Unique_mRNAseq_Cells_RAW_counts.csv")


df <- read.csv("Unique_mRNAseq_Cells_RAW_counts.csv", header = TRUE, row.names = 1)[,-c(1)]
cleanDat <- df
dim(cleanDat)
...
numericMeta <- read.csv("mRNAseq_Cells_RAW_Traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_cells_1","control_cells_2","control_cells_3","LPS_cells_1" ,"LPS_cells_2","LPS_cells_3","IL10_cells_1",   
                        "IL10_cells_2","IL10_cells_3","TGFB_cells_1","TGFB_cells_2","TGFB_cells_3")
#########################################################CHANGE##############################################################################
