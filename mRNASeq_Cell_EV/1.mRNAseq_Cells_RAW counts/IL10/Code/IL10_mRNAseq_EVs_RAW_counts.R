...
##############CHECK##############
#Comparison:Control VS IL10: Update your script name and comparison here + Double check the folder you are in to avoid mixing diff ex results!
rm(list = ls())
dev.off()
cat("\014")
...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/1.mRNAseq_Cells_RAW counts/IL10/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("IL10_mRNAseq_Cells_RAW_counts.csv", header = TRUE)
...
write.csv(df, file = "Unique_IL10_mRNAseq_Cells_RAW_counts.csv")


df <- read.csv("Unique_IL10_mRNAseq_Cells_RAW_counts.csv", header = TRUE, row.names = 1)[,-c(1)]
cleanDat <- df
dim(cleanDat)
...
numericMeta <- read.csv("IL10_mRNAseq_Cells_RAW_Traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_cells_1","control_cells_2","control_cells_3","IL10_cells_1","IL10_cells_2","IL10_cells_3" )
#########################################################CHANGE##############################################################################
...
EnhancedVolcano(res05,
                ...
                x = 'log2FoldChange',
                y = 'pvalue',
                title = 'Treatment_IL10_vs_Control',
                pCutoff = 0.05,
                FCcutoff = 1,
...
pheatmap(matrixx, annotation_col=annotation_data, show_rownames = T,cluster_cols= FALSE, main = "Treatment_IL10_vs_Control") #true / False to toggle supervised and unsupervised clustering
#########################################################CHANGE##############################################################################
