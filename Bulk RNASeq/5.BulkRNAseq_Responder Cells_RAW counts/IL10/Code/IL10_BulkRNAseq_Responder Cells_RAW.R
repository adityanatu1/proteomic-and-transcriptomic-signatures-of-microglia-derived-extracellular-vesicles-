...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/5.BulkRNAseq_Responder Cells_RAW counts/IL10/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("IL10_BulkRNAseq_Responder Cells_RAW_counts.csv", header = TRUE, row.names = 1)
numericMeta <- read.csv("IL10_BulkRNAseq_Responder Cells_RAW_traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_Evs_Tx_1","control_Evs_Tx_2","control_Evs_Tx_3","IL10_Evs_Tx_1","IL10_Evs_Tx_2","IL10_Evs_Tx_3")
#########################################################CHANGE##############################################################################
...
EnhancedVolcano(res05,
                ...
                x = 'log2FoldChange',
                y = 'pvalue',
                title = 'Treatment IL10 vs Control',
                pCutoff = 0.05,
                FCcutoff = 1,
...
pheatmap(matrixx, annotation_col=annotation_data, show_rownames = T,cluster_cols= FALSE, main = "Treatment IL10 vs Control") #true / False to toggle supervised and unsupervised clustering
#########################################################CHANGE##############################################################################
