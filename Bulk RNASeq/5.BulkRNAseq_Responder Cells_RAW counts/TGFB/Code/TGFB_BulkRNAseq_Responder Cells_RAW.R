...
##############CHECK##############
#Comparison:Control VS TGFB : Update your script name and comparison here + Double check the folder you are in to aviod mixing diff ex results!
rm(list = ls())
dev.off()
cat("\014")
...
##############CHECK##############
setwd("/Users/adityanatu/Library/Mobile Documents/com~apple~CloudDocs/Project_2023/Juliet_Data_Analysis/EV_RNASeq/5.BulkRNAseq_Responder Cells_RAW counts/TGFB/")
getwd()
...
##############CHECK##############
cleanDat <- read.csv("TGFB_BulkRNAseq_Responder Cells_RAW_counts.csv", header = TRUE, row.names = 1)
numericMeta <- read.csv("TGFB_BulkRNAseq_Responder Cells_RAW_traits.csv", header = TRUE, row.names = 1)

colnames(cleanDat)
rownames(numericMeta)

##############CHECK##############
colnames(cleanDat) <- c("control_Evs_Tx_1","control_Evs_Tx_2","control_Evs_Tx_3","TGFB_Evs_Tx_1","TGFB_Evs_Tx_2","TGFB_Evs_Tx_3")
#########################################################CHANGE##############################################################################
